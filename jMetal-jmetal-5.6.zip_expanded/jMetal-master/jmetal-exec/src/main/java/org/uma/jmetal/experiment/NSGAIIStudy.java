...
public class NSGAIIStudy {
  ...
  public static void main(String[] args) throws IOException {
    ...
    String experimentBaseDirectory ="C:\\otimizacao";

    List<ExperimentProblem<BinarySolution>> problemList = new ArrayList<>();
    
    //Falta adicionar o problema aqui
    problemList.add(new ExperimentProblem<>(new OneZeroMax()));
      ...
      for (int run = 0; run < INDEPENDENT_RUNS; run++) {
        for (int i = 0; i < problemList.size(); i++) {
          ...
          Algorithm<List<BinarySolution>> algorithm = new NSGAIIBuilder<>(
                  ...
                  new BitFlipMutation(1.0 / problemList.get(i).getProblem().getNumberOfVariables()))
